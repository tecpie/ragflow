...
## Git branch policy（强制）

**同步测试机必须以本地 `dev` 分支为源。**

原因：debug 挂载的是整树代码。从 feature 分支增量/全量同步会覆盖 `dev` 上已合入、但 feature 未包含的改动（例如 `document_ids` 文档范围限制），导致线上回归。

| 场景 | 做法 |
|------|------|
| 当前就在 `dev` | 直接跑 sync 脚本 |
| 当前在其他分支 | **先把要发布的改动合并进 `dev`**，再 `git checkout dev`，然后同步 |
| 紧急只想传单个文件、且确认不会盖掉 `dev` 独有逻辑 | 仅允许加 `--allow-non-dev`（须在回复里说明风险） |

Agent 执行同步前必须：

1. `git branch --show-current` —— 不是 `dev` 则 **不要直接同步**。
2. 协助把相关提交 merge / cherry-pick 到 `dev`（有冲突先解决）。
3. 在 `dev` 上再跑 `sync-to-test-server`。
4. 禁止用「当前 feature 工作区里缺了某功能的文件」去覆盖服务器上仍依赖该功能的文件。

脚本默认也会拦截非 `dev` 同步；只有显式 `--allow-non-dev` 才放行。
...
0. **Confirm current branch is `dev`.** If not, merge/cherry-pick into `dev`, checkout `dev`, then sync. Do not sync from a feature branch (除非用户明确要求并使用 `--allow-non-dev`).
...
| `--allow-non-dev` | Allow sync while not on `dev` (emergency only; default blocked) |
