...
type chatSessionStore interface {
	...
	ListByChatID(ctx context.Context, db *gorm.DB, chatID, sessionID, name, filterUserID string, terms []dao.OrderTerm, page, pageSize int, includeHistory ...bool) ([]*entity.ChatSession, error)
...
// filterUserID, when non-empty, restricts results to that end-user's sessions
// (mirrors Python list_sessions ?user_id=).
func (s *ChatSessionService) ListChatSessions(ctx context.Context, userID, chatID, sessionID, name, filterUserID string, terms []dao.OrderTerm, page, pageSize int, includeHistory ...bool) (*ListChatSessionsResponse, error) {
	// Get user's tenants
	tenantIDs, err := s.userTenantDAO.GetTenantIDsByUserID(ctx, dao.DB, userID)
	if err != nil {
		return nil, err
	...
	// List chat sessions
	sessions, err := s.chatSessionDAO.ListByChatID(ctx, dao.DB, chatID, sessionID, name, filterUserID, terms, page, pageSize, includeHistory...)
	if err != nil {
		return nil, err
	}
...
func (s *ChatSessionService) CreateSession(ctx context.Context, userID, chatID string, req map[string]interface{}) (*ChatSessionPayload, common.ErrorCode, error) {
	...
	// Prefer end-user id from request body; fall back to API-token owner (tenant).
	sessionUserID := userID
	if raw, ok := req["user_id"].(string); ok {
		if trimmed := strings.TrimSpace(raw); trimmed != "" {
			sessionUserID = trimmed
		}
	}
...
func (s *ChatSessionService) ChatCompletions(
	...
	if chatID != "" {
		...
		if sessionID != "" {
			...
			if raw, ok := kwargs["user_id"].(string); ok {
				if trimmed := strings.TrimSpace(raw); trimmed != "" {
					sessionOwnerID = trimmed
				}
			}
			session, err = s.createSessionForCompletion(ctx, chatID, dialog, sessionOwnerID)
			if err != nil {
				return fail(err)
			}
