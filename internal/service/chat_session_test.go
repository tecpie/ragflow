...
func (f *fakeSessionStore) ListByChatID(ctx context.Context, db *gorm.DB, chatID, sessionID, name, filterUserID string, terms []dao.OrderTerm, page, pageSize int, includeHistory ...bool) ([]*entity.ChatSession, error) {
	...
	for _, s := range f.sessions {
		...
		if filterUserID != "" {
			if s.UserID == nil || *s.UserID != filterUserID {
				continue
			}
		}
