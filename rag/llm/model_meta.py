...
class Base(ABC):
    ...
    async def _get_raw_model_list(self):
        ...
        if not url:
            logging.warning("model list skipped: base_url missing factory=%s", self.__class__.__name__)
            return None
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers={"Authorization": f"Bearer {self._get_api_key()}"}) as resp:
                    if resp.status != 200:
                        ...
                        logging.warning(
                            "model list request failed factory=%s url=%s status=%s body=%s",
            ...
            logging.exception("model list request error factory=%s url=%s", self.__class__.__name__, url)
            return None
