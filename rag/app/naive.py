...
from deepdoc.parser.figure_parser import (
    VisionFigureParser,
    enhance_signature_sections_with_vision,
...
def by_deepdoc(filename, binary=None, from_page=0, to_page=MAXIMUM_PAGE_NUMBER, lang="Chinese", callback=None, pdf_cls=None, **kwargs):
    ...
    try:
        sections = enhance_signature_sections_with_vision(
            pdf_parser,
            sections,
            callback=callback,
            lang=lang,
        ...
        if callback:
            callback(0.8, f"Signature visual model error: {e}. Skipping signature vision enhancement.")
        logging.exception("Signature vision enhancement failed")
