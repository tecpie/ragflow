# base stage
FROM registry.cn-hangzhou.aliyuncs.com/tecpie/ubuntu:24.04 AS base
USER root
SHELL ["/bin/bash", "-c"]
...
RUN --mount=type=secret,id=gitee_token \
    ...
    if [ "$NEED_MIRROR" == "1" ]; then \
        GITEE_TOKEN=$(cat /run/secrets/gitee_token 2>/dev/null || echo "$GITEE_TOKEN"); \
...
RUN --mount=type=bind,from=registry.cn-hangzhou.aliyuncs.com/tecpie/ragflow_deps:latest,source=/chrome-linux64-121-0-6167-85,target=/chrome-linux64.zip \
    unzip /chrome-linux64.zip && \
    mv chrome-linux64 /opt/chrome && \
    ln -s /opt/chrome/chrome /usr/local/bin/
RUN --mount=type=bind,from=registry.cn-hangzhou.aliyuncs.com/tecpie/ragflow_deps:latest,source=/chromedriver-linux64-121-0-6167-85,target=/chromedriver-linux64.zip \
