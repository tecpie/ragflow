...
class FileService(CommonService):
    ...
    def upload_document(self, kb, file_objs, user_id, src="local", parent_path: str | None = None, parser_config_override: dict | None = None):
        ...
        for file in file_objs:
            ...
            try:
                logging.info(
                    "[upload_filename_trace] stage=FileService.upload_document.entry "
                    "raw_filename=%r kb_id=%s",
                    file.filename,
                    kb.id,
                )
                ...
                logging.info(
                    "[upload_filename_trace] stage=FileService.upload_document.resolved "
                    "raw_filename=%r stored_name=%r suffix=%r filetype=%r parser_id=%s location=%r",
                    file.filename,
                    filename,
                    Path(filename).suffix.lstrip("."),
                    filetype,
                    self.get_parser(filetype, filename, kb.parser_id),
                    filename if not safe_parent_path else f"{safe_parent_path}/{filename}",
                )
    ...
    def upload_info(user_id, file, url: str|None=None):
        def structured(filename, filetype, blob, content_type):
            ...
            logging.info(
                "[upload_filename_trace] stage=FileService.upload_info.structured "
                "stored_name=%r suffix=%r filetype=%r content_type=%r tenant_id=%s",
                filename,
        ...
        logging.info(
            "[upload_filename_trace] stage=FileService.upload_info.entry "
            "raw_filename=%r content_type=%r tenant_id=%s",
            file.filename,
            file.content_type,
            user_id,
        )
