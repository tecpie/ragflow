...
@manager.route("/mcp_server/list", methods=["POST"])  # noqa: F821
@token_required
async def list_mcp(tenant_id) -> Response:
    ...
    try:
        servers = MCPServerService.get_servers(tenant_id, mcp_ids, 0, 0, orderby, desc, keywords) or []
        total = len(servers)
...
@manager.route("/mcp_server/detail", methods=["GET"])  # noqa: F821
@token_required
async def detail(tenant_id) -> Response:
    mcp_id = request.args["mcp_id"]
    try:
        mcp_server = MCPServerService.get_or_none(id=mcp_id, tenant_id=tenant_id)
...
@manager.route("/mcp_server/create", methods=["POST"])  # noqa: F821
@token_required
@validate_request("name", "url", "server_type")
async def create(tenant_id) -> Response:
    req = await get_request_json()
    ...
    e, _ = MCPServerService.get_by_name_and_tenant(name=server_name, tenant_id=tenant_id)
    if e:
...
@manager.route("/mcp_server/update", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_id")
async def update(tenant_id) -> Response:
    req = await get_request_json()

    mcp_id = req.get("mcp_id", "")
    e, mcp_server = MCPServerService.get_by_id(mcp_id)
    if not e or mcp_server.tenant_id != tenant_id:
        return get_data_error_result(message=f"Cannot find MCP server {mcp_id} for user {tenant_id}")
    ...
    try:
        ...
        if not MCPServerService.filter_update([MCPServer.id == mcp_id, MCPServer.tenant_id == tenant_id], req):
...
@manager.route("/mcp_server/rm", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_ids")
async def rm(tenant_id) -> Response:
    req = await get_request_json()
    mcp_ids = req.get("mcp_ids", [])
...
@manager.route("/mcp_server/import", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcpServers")
async def import_multiple(tenant_id) -> Response:
    req = await get_request_json()
    ...
    try:
        for server_name, config in servers.items():
            ...
            while True:
                e, _ = MCPServerService.get_by_name_and_tenant(name=new_name, tenant_id=tenant_id)
                if not e:
                    break
...
@manager.route("/mcp_server/export", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_ids")
async def export_multiple(tenant_id) -> Response:
    req = await get_request_json()
    mcp_ids = req.get("mcp_ids", [])
    ...
    try:
        ...
        for mcp_id in mcp_ids:
            ...
            if e and mcp_server.tenant_id == tenant_id:
...
@manager.route("/mcp_server/list_tools", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_ids")
async def list_tools(tenant_id) -> Response:
    req = await get_request_json()
    mcp_ids = req.get("mcp_ids", [])
    if not mcp_ids:
        return get_data_error_result(message="No MCP server IDs provided.")
...
@manager.route("/mcp_server/test_tool", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_id", "tool_name", "arguments")
async def test_tool(tenant_id) -> Response:
    req = await get_request_json()
    mcp_id = req.get("mcp_id", "")
    if not mcp_id:
...
@manager.route("/mcp_server/cache_tools", methods=["POST"])  # noqa: F821
@token_required
@validate_request("mcp_id", "tools")
async def cache_tool(tenant_id) -> Response:
    req = await get_request_json()
    mcp_id = req.get("mcp_id", "")
    if not mcp_id:
        return get_data_error_result(message="No MCP server ID provided.")
    ...
    if not MCPServerService.filter_update([MCPServer.id == mcp_id, MCPServer.tenant_id == tenant_id], {"variables": variables}):
        return get_data_error_result(message="Failed to updated MCP server.")

    return get_json_result(data=tools)


@manager.route("/mcp_server/test_mcp", methods=["POST"])  # noqa: F821
@token_required
@validate_request("url", "server_type")
async def test_mcp(tenant_id) -> Response:
    req = await get_request_json()
