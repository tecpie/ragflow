...
from common.constants import FileSource, LLMType, ParserType, RetCode, TaskStatus, PipelineTaskType
...
@token_required
def list_docs_multi(tenant_id):
    """
    List documents across multiple datasets, with unified pagination and sorting.
    ---
    tags:
      - Documents
    security:
    ...
    parameters:
      - in: query
        ...
        items:
          type: string
        required: true
        description: List of dataset IDs to query.
      - in: query
        name: id
        type: string
        required: false
    ...
    """
    q = request.args
    dataset_ids = q.getlist("dataset_ids")
    if not dataset_ids:
    ...
    # 在 DocumentService 层使用 kb_id in (dataset_ids) 做查询，并直接按 page/page_size 分页
    ...
    # 字段映射与 run 状态映射（与 list_docs 保持一致）
    key_mapping = {
        "chunk_num": "chunk_count",
        "kb_id": "dataset_id",
        "token_num": "token_count",
...
@manager.route("/datasets/<dataset_id>/unbind_task", methods=["DELETE"])  # noqa: F821
@token_required
async def unbind_dataset_task(tenant_id, dataset_id):
    """
    Cancel and unbind a dataset pipeline task.
    ---
    tags:
      - Datasets
    security:
    ...
    parameters:
      - in: path
        ...
        description: ID of the dataset.
      - in: query
        name: pipeline_task_type
        type: string
        required: true
        description: Pipeline task type. One of `graphrag`, `raptor`, `mindmap`.
      - in: header
        name: Authorization
        type: string
        required: true
    ...
    responses:
      200:
        description: Task unbound successfully.
        schema:
          type: object
    """
    if not KnowledgebaseService.accessible(kb_id=dataset_id, user_id=tenant_id):
    ...
    if pipeline_task_type not in [PipelineTaskType.GRAPH_RAG, PipelineTaskType.RAPTOR, PipelineTaskType.MINDMAP]:
    ...
    kb_task_id_field = ""
    kb_task_finish_at = ""
    if pipeline_task_type == PipelineTaskType.GRAPH_RAG:
        kb_task_id_field = "graphrag_task_id"
        kb_task_finish_at = "graphrag_task_finish_at"
        cancel_task(kb.graphrag_task_id)
        settings.docStoreConn.delete(
            {"knowledge_graph_kwd": ["graph", "subgraph", "entity", "relation"]},
            search.index_name(kb.tenant_id),
            dataset_id,
        )
    elif pipeline_task_type == PipelineTaskType.RAPTOR:
        kb_task_id_field = "raptor_task_id"
        kb_task_finish_at = "raptor_task_finish_at"
        cancel_task(kb.raptor_task_id)
        settings.docStoreConn.delete(
            {"raptor_kwd": ["raptor"]},
            search.index_name(kb.tenant_id),
            dataset_id,
        ...
        cancel_task(kb.mindmap_task_id)

    ok = KnowledgebaseService.update_by_id(dataset_id, {kb_task_id_field: "", kb_task_finish_at: None})
    if not ok:
