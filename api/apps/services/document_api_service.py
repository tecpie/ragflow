...
def update_document_status_only(status: int, doc, kb):
    ...
    if doc.status is None or (int(doc.status) != status):
        try:
            ...
            # Flip source chunks (parents stay unavailable) and knowledge-
            # compilation rows together so artifacts produced while the document
            # was disabled become retrievable on approval/enable.
            sync_document_source_chunk_availability(doc.id, kb.tenant_id, doc.kb_id, status)
            settings.docStoreConn.update(
                {"doc_id": doc.id, "exists": "compile_kwd"},
