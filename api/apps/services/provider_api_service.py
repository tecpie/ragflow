...
async def list_provider_models(provider_id_or_name: str, api_key: str = None, base_url: str = None):
    ...
    if provider_name in ModelMeta:
        try:
            ...
            logging.exception(
                "list_provider_models: failed to list remote models provider=%s base_url=%s",
                provider_name,
                model_base_url,
            )
            remote_models = []
    ...
    if not static_llms and not remote_models:
        logging.warning(
            "list_provider_models: no models provider=%s base_url=%s static=%d remote=%d",
            provider_name,
            model_base_url,
            len(static_llms),
            len(remote_models),
            ...
            try:
                if provider_name in ModelMeta:
                    ...
                    if remote_models:
                        ...
                        logging.warning(
                            "verify_api_key: remote model list empty provider=%s base_url=%s",
                            provider_name,
                            model_base_url,
                        )
            except Exception:
                logging.exception(
                    "verify_api_key: failed to list remote models provider=%s base_url=%s",
                    provider_name,
                    model_base_url,
                )
            if not factory_llms:
