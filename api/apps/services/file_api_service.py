...
async def upload_file(tenant_id: str, pf_id: str, file_objs: list):
    ...
    for file_obj in file_objs:
        ...
        raw_name = file_obj_names[file_len - 1]
        logging.info(
            "[upload_filename_trace] stage=file_api_service.upload_file.entry "
            "raw_filename=%r parsed_path=%r tenant_id=%s parent_id=%s",
            file_obj.filename,
            file_obj_names,
            tenant_id,
            last_folder.id,
        )
        filetype = filename_type(raw_name)
        location = raw_name
        ...
        filename = await thread_pool_exec(
            duplicate_name, FileService.query, name=raw_name, parent_id=last_folder.id
        )
        logging.info(
            "[upload_filename_trace] stage=file_api_service.upload_file.resolved "
            "raw_filename=%r stored_name=%r suffix=%r filetype=%r location=%r",
            file_obj.filename,
            filename,
